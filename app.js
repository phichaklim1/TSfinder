/* Thai Stock Finder — SIMPLE v1 (Local + Web; Choose File) */
const TSF = (() => {
    ...
    PICK().addEventListener('change', e=>{
      ...
      r.onload = async ev => {
        try{
          ...
          STATUS().textContent='อ่านไฟล์ไม่สำเร็จ: '+(err && err.message ? err.message : err);
        }
      };
  ...
  function start(){
    wireUI();
    STATUS().textContent='เลือกไฟล์ database.xlsx แล้วพิมพ์ย่อหุ้นเพื่อค้นหา';
  }
